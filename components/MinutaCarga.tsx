...
export default function MinutaCarga({ platos }: { platos: Array<{ plato: string; tiene_receta: boolean }> }) {
  ...
  const errors = useMemo(() => (rows.length ? validarFilasMinuta(rows) : []), [rows]);
  const warnings = useMemo(() => (rows.length ? advertenciasFilasMinuta(rows) : []), [rows]);
  ...
  function crearMatriz() {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(inicio) || !/^\d{4}-\d{2}-\d{2}$/.test(fin) || fin < inicio) {
      setMessage('Rango de fechas inválido.');
      return;
    }
  ...
  const tieneError = (fecha: string, servicio: string, opcion: string) => {
    const idx = rows.findIndex(r => r.fecha === fecha && r.servicio === servicio && r.tipo_opcion === opcion);
    return idx < 0 || errors.some(e => e.fila === idx + 1);
  };
  ...
  return (
    <div className="mt-4 rounded-2xl border bg-white p-4">
      ...
      <div className="mt-4 grid gap-3 rounded-xl bg-[#F6F3EA] p-4 sm:grid-cols-[1fr_1fr_auto]">
        <label className="text-sm font-bold">Desde
          <input type="date" value={inicio} onChange={e => setInicio(e.target.value)} className="mt-1 w-full rounded-lg border bg-white p-2 font-normal" />
        </label>
        <label className="text-sm font-bold">Hasta
          <input type="date" value={fin} onChange={e => setFin(e.target.value)} className="mt-1 w-full rounded-lg border bg-white p-2 font-normal" />
        </label>
      ...
      <datalist id="platos-minuta">
        {platos.map(item => <option key={item.plato} value={item.plato} />)}
      </datalist>
        ...
        <div className="mt-5">
          ...
          {semanas.length > 1 && (
            <div className="mb-3 flex flex-wrap gap-2">
              {semanas.map((s, i) => (
                <button key={i} type="button" onClick={() => setSemanaActiva(i)} className={`rounded-lg px-3 py-2 text-sm font-black ${semanaActiva === i ? 'bg-[#0E2A23] text-white' : 'border bg-white'}`}>
                  Semana {i + 1}
                  <span className="ml-1 text-[10px] opacity-70">{nombreFecha(s[0])}–{nombreFecha(s[s.length - 1])}</span>
                </button>
              ))}
            </div>
          )}
            ...
            <table className="min-w-max border-collapse text-xs">
              <thead>
                <tr className="bg-[#EAF0F4]">
                  ...
                  {semana.map(fecha => (
                    <th key={fecha} className={`min-w-[220px] border p-2 text-center ${fecha === hoy ? 'bg-[#1DB954]/20' : ''}`}>
                      <div className="font-black">{nombreFecha(fecha)}</div>
                      {fecha === hoy && <div className="text-[10px] font-black text-[#0E2A23]">HOY</div>}
                    </th>
                  ))}
                </tr>
              </thead>
                ...
                {SERVICIOS.flatMap(servicio =>
                  OPCIONES.map((opcion, oi) => (
                    <tr key={`${servicio}-${opcion}`}>
                      <th className={`sticky left-0 z-10 border p-2 text-left font-black ${servicio === 'Almuerzo' ? 'bg-[#EAF7EF]' : 'bg-[#EDF3FA]'}`}>
                        {oi === 0 ? servicio : ''}
                      </th>
                      ...
                      {semana.map(fecha => (
                        <td key={`${fecha}-${servicio}-${opcion}`} className={`border p-1 ${fecha === hoy ? 'bg-[#1DB954]/5' : ''}`}>
                          <textarea
                            value={get(fecha, servicio, opcion)}
                            onChange={e => patchCell(fecha, servicio, opcion, e.target.value)}
                            rows={4}
                            className={`w-full min-w-[210px] resize-y rounded border p-2 text-sm leading-5 ${tieneError(fecha, servicio, opcion) ? 'border-red-500 bg-red-50' : 'border-[#A6B0AA]/45 bg-white'}`}
      ...
      {errors.length > 0 && (
        <div className="mt-3 rounded-lg border border-red-300 bg-red-50 p-3 text-sm text-red-900">
          <p className="font-black">Errores que sí deben corregirse ({errors.length})</p>
          <ul className="mt-2 list-disc pl-5">
            {errors.map((e, i) => {
              const r = rows[e.fila - 1];
              return <li key={`${e.fila}-${e.campo}-${i}`}><b>Fila {e.fila}</b>{r ? ` · ${r.fecha || 'sin fecha'} · ${r.servicio || 'sin servicio'} · ${r.tipo_opcion || 'sin opción'}` : ''}: {e.mensaje}</li>;
            })}
          </ul>
        </div>
      )}
      ...
      {warnings.length > 0 && (
        <div className="mt-3 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">
          <p className="font-black">Advertencias de minuta ({warnings.length}) — NO bloquean publicación</p>
          <ul className="mt-2 list-disc pl-5">
            {warnings.map((w, i) => {
              const r = rows[w.fila - 1];
              return <li key={`${w.fila}-${i}`}>{r ? `${r.fecha} · ${r.servicio} · ${r.tipo_opcion}: ` : ''}{w.mensaje}</li>;
            })}
          </ul>
        </div>
      )}
      ...
      {rows.some(row => {
        const item = platos.find(plato => plato.plato.toLocaleLowerCase('es-CL') === row.plato.trim().toLocaleLowerCase('es-CL'));
        return item && !item.tiene_receta;
      }) && (
        <p className="mt-3 rounded-lg bg-amber-50 p-3 text-sm text-amber-800">Advertencia: uno o más platos no tienen receta activa. Esto no impide publicar la minuta; solo limita el cálculo futuro de insumos.</p>
      )}
