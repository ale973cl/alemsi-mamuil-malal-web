  ...
  return <div className="mx-auto max-w-6xl px-3 py-4 sm:px-6 sm:py-8">
    <section className="overflow-hidden rounded-2xl border border-[#A6B0AA]/35 bg-[#FFFDF8] shadow-[0_12px_36px_rgba(14,42,35,0.08)] sm:rounded-3xl">
      ...
      <div className="p-4 sm:p-8">
        ...
        {etapa==='resultado'&&resultado?.ok&&<div className="mx-auto max-w-2xl text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#1DB954] text-2xl font-black">✓</div><p className="mt-4 text-xs font-bold uppercase tracking-[.2em] text-[#1DB954]">Reserva confirmada</p><h3 className="mt-1 text-3xl font-black">{resultado.result.codigoReserva}</h3><p className="mt-2 text-sm text-[#6B7570]">Referencia: {resultado.result.referencia}</p>{resultado.result.total>0&&<p className="mt-4 text-xl font-extrabold">Total: {money(resultado.result.total)}</p>}<CorreoSpamAviso className="mt-5"/>{resultado.result.pagoToken&&<div className="mt-6 flex flex-col items-center gap-3"><a href="/mis-reservas" className="inline-flex min-h-12 items-center rounded-xl bg-[#0E2A23] px-6 font-extrabold text-white">Gestionar en Mis reservas</a><a href={`/comprobante/${encodeURIComponent(resultado.result.pagoToken)}`} className="text-sm font-bold underline">Acceso directo opcional al comprobante</a></div>}</div>}
