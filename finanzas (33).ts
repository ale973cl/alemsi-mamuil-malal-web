...
export function estadoBandeja(row:FilaFinanzas):EstadoBandeja{
  ...
  if(pago==='pagado'||comprobante==='VALIDADO') return 'validados';
  // OBSERVADO se conserva solo como compatibilidad histórica; las nuevas acciones financieras son Aceptar/Rechazar.
  if(pago==='rechazado'||pago==='observado'||comprobante==='RECHAZADO'||comprobante==='OBSERVADO') return 'rechazados';
