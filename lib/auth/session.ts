...
export type Rol = 'AdminTotal'|'AdminCasino'|'Finanzas'|'Cocina'|'Coordinacion'|'Gerencia'|'Bodega'|'Operaciones';
export type SessionUser = { username:string; nombre:string; rol:Rol; correo?:string };
...
export async function getSession():Promise<SessionUser|null>{ try{ const raw=(await cookies()).get(COOKIE)?.value; if(!raw) return null; const [body,sig]=raw.split('.'); if(!body||!sig) return null; const expected=sign(body); if(!crypto.timingSafeEqual(Buffer.from(sig),Buffer.from(expected))) return null; return JSON.parse(Buffer.from(body,'base64url').toString()) as SessionUser; }catch{return null;} }
