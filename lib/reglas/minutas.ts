...
export type AdvertenciaFilaMinuta={fila:number;campo:string;mensaje:string};
...
export function advertenciasFilasMinuta(rows:FilaMinutaInput[]):AdvertenciaFilaMinuta[]{
  const advertencias:AdvertenciaFilaMinuta[]=[];
  const platosPorServicio=new Map<string,Map<string,{plato:string;fila:number}>>();
  rows.forEach((original,index)=>{
    const row=normalizarFilaMinuta(original); const fila=index+1;
    if(!row.plato||!['OPCION 1','OPCION 2'].includes(row.tipo_opcion)) return;
    ...
    if(contraparte&&contraparte.plato.toLocaleLowerCase('es-CL')===row.plato.toLocaleLowerCase('es-CL')) advertencias.push({fila,campo:'plato',mensaje:`Opción 1 y Opción 2 repiten plato (fila ${contraparte.fila}). Se permite publicar si Admin Casino lo confirma.`});
