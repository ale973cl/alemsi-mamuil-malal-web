...
function timeout<T>(promise: Promise<T>, ms: number): Promise<T> {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(
      () => reject(new SafeSmtpError('timeout')),
      ms
    );
...
function openConnection(
  ...
  return new Promise((resolve, reject) => {
    ...
    const fail = () =>
      reject(new SafeSmtpError('connection'));
...
function responseReader(socket: net.Socket | tls.TLSSocket) {
  ...
  return {
    ...
    async read(
      expected: number,
      errorType: SmtpErrorType = 'protocol'
    ) {
      for (;;) {
        const item = await iterator.next();

        if (item.done) {
          throw new SafeSmtpError(errorType);
        }

        const match = /^(\d{3})([ -])/.exec(
          String(item.value)
        );
...
async function authenticatedSession() {
  ...
  let responses:
    | ReturnType<typeof responseReader>
    | undefined;

  try {
    socket = await timeout(
      openConnection(config.server, config.port),
      10_000
    );
    ...
    await timeout(
      responses.read(334, 'authentication'),
      10_000
    );
    ...
    await timeout(
      responses.read(235, 'authentication'),
      10_000
    );
...
export async function verificarTransporteSmtp(): Promise<SmtpVerification> {
  ...
  try {
    ...
    let responses = responseReader(socket);
    reader = responses.lines;
    ...
    return {
      ok: true,
      status: 'SMTP CONNECTED / AUTH OK',
    };
  } catch (error) {
    return {
      ok: false,
      errorType:
        error instanceof SafeSmtpError
          ? error.errorType
          : 'protocol',
    };
  } finally {
...
export async function enviarCorreoSmtp(input: {
  ...
  try {
    ...
    command(
      socket,
      `MAIL FROM:<${session.config.user}>`
    );

    await timeout(
      session.responses.read(250),
      10_000
    );
    ...
    socket.write(
      `From: ALEMSI <${session.config.user}>\r\n` +
      `To: <${destinatario}>\r\n` +
