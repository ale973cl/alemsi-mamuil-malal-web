...
export function correoHtmlEstandar(title:string,content:string,eyebrow='CASINO MAMUIL',headerHtml=''){
  const banner=String(process.env.SEPTEMBER_EMAIL_BANNER_URL||'').trim();
  const top=headerHtml
    ? `<tr><td style="padding:0;text-align:center">${headerHtml}</td></tr>`
    : banner
