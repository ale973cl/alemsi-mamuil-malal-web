...
export async function contarActividadFinanzas(){
  const rows=await query<{por_validar:number;sin_comprobante:number}>(
    `WITH reservas AS (
       SELECT s.codigo_reserva,
              UPPER(COALESCE(MAX(s.estado_pago),'')) estado_pago,
              UPPER(COALESCE(MAX(cp.estado),'')) comprobante_estado,
              MAX(cp.id) comprobante_id,
              SUM(COALESCE(s.precio_aplicado,s.precio,0)) total
         FROM solicitudes s
         LEFT JOIN LATERAL (
     ...
     SELECT COUNT(*) FILTER (WHERE total>0 AND estado_pago NOT IN ('PAGADO','VALIDADO') AND comprobante_id IS NOT NULL AND comprobante_estado='RECIBIDO')::int por_validar,
            COUNT(*) FILTER (WHERE total>0 AND estado_pago NOT IN ('PAGADO','VALIDADO') AND comprobante_id IS NULL)::int sin_comprobante
       FROM reservas`,
  );
  const porValidar=Number(rows[0]?.por_validar||0);
  const sinComprobante=Number(rows[0]?.sin_comprobante||0);
...
export async function obtenerDatosNotificacionFinanzas(codigo:string){
  const rows=await query<{codigo_reserva:string;rut:string;nombre:string;institucion:string;correo:string;pago_token:string;metodo_pago:string;total:number}>(
    `SELECT s.codigo_reserva,
            MAX(s.rut) rut,
...
export async function validarPago(codigo:string,estado:'Pagado'|'Rechazado',usuario:string,motivo:string){
  ...
  await inTransaction(async c=>{
    ...
    const comprobante=await c.query<{id:number;estado:string}>(`SELECT id,estado FROM comprobantes_pago WHERE referencia_reserva=$1 ORDER BY id DESC LIMIT 1 FOR UPDATE`,[codigo]);
    ...
    await c.query(`UPDATE solicitudes SET estado_pago=$1,motivo_estado_pago=$2,fecha_modificacion=$3,modificado_por=$4 WHERE codigo_reserva=$5`,[estado,motivoLimpio,ahora,usuario,codigo]);
    await c.query(`UPDATE comprobantes_pago SET estado=$1,validado_por=$2,fecha_validacion=$3,observacion_validacion=$4 WHERE id=$5`,[estado==='Pagado'?'VALIDADO':'RECHAZADO',usuario,ahora,motivoLimpio,comprobante.rows[0].id]);
...
export async function validarPagoSinComprobante(codigo:string,usuario:string,medio:string,motivo:string){
  const medioLimpio=medio.trim();const motivoLimpio=motivo.trim();
  ...
  await inTransaction(async c=>{
    const ahora=new Date().toISOString();
    const reserva=await c.query<{estado_pago:string|null}>(`SELECT estado_pago FROM solicitudes WHERE codigo_reserva=$1 LIMIT 1 FOR UPDATE`,[codigo]);
    if(!reserva.rows[0]) throw new Error('Reserva no encontrada.');
    const detalle=`Validación manual (${medioLimpio}): ${motivoLimpio}`;
    await c.query(`UPDATE solicitudes SET estado_pago='Pagado',motivo_estado_pago=$1,fecha_modificacion=$2,modificado_por=$3 WHERE codigo_reserva=$4`,[detalle,ahora,usuario,codigo]);
