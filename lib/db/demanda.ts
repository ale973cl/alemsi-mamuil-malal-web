  ...
  return query<DemandaRangoRow>(
    `WITH base AS (
       SELECT DISTINCT ON (s.rut,s.fecha,s.servicio)
              s.id,s.rut,s.fecha,s.servicio,COALESCE(NULLIF(TRIM(s.institucion),''),'Sin institución') institucion,
     ...
     SELECT fecha,servicio,tipo_opcion,COALESCE(plato,'') plato,institucion,COUNT(*)::int cantidad
       FROM base
