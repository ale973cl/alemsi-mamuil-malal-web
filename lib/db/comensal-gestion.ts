...
import { notificarIngresoReclamoInterno, obtenerRuteoReclamo } from '@/lib/reclamos-routing';
...
async function notificarIngresoInternoLegacy(registro:{id:number;rut:string;nombre:string;tipo:string;categoria:string;mensaje:string;fecha:string}){
  ...
  if(!destinatarios.length){console.info('RECLAMO_INTERNAL_SMTP_SKIP','sin_destinatarios_configurados');return;}
  const folio=`R-${String(registro.id).padStart(6,'0')}`;
  const text=[`Nuevo caso ${folio}`,`Tipo: ${registro.tipo}`,`Categoría: ${registro.categoria}`,`Comensal: ${registro.nombre}`,`RUT: ${registro.rut}`,`Fecha: ${fechaHoraVisibleChile(new Date(registro.fecha))}`,`Mensaje: ${registro.mensaje}`,'El caso quedó registrado en ALEMSI Casino para seguimiento.'].join('\n\n');
  for(const to of destinatarios){const result=await enviarCorreoSmtp({to,subject:`ALEMSI · Nuevo ${registro.tipo} · ${folio}`,text});if(result.ok) console.info('RECLAMO_INTERNAL_SMTP_OK',to,folio);else console.error('RECLAMO_INTERNAL_SMTP_ERROR',to,folio,result.errorType);}
}
...
export async function guardarReclamo(rutInput:string,tipo:string,categoria:string,mensaje:string){
  ...
  let ruteo:{principal:{rol:string}|null}|null=null;
  try{ruteo=await obtenerRuteoReclamo(categoria);}catch(error){console.error('RECLAMO_ROUTING_CONFIG_ERROR',error instanceof Error?error.message:'unknown');}
  const areaActual=ruteo?.principal?.rol||'AdminCasino';
  const rows=await query<any>(`INSERT INTO reclamos_sugerencias (rut,nombre,tipo,categoria,mensaje,fecha,estado,area_actual) VALUES ($1,$2,$3,$4,$5,$6,'Pendiente',$7) RETURNING id`,[rut,c.nombre,tipo,categoria,mensaje.trim(),fecha,areaActual]);
  ...
  try{await agregarMovimientoReclamo({reclamoId:registro.id,actor:'Sistema',actorRol:'AdminCasino',accion:'INGRESO_COMENSAL',destinoRol:areaActual,mensaje:'Caso recibido desde portal de comensales.',estado:'Pendiente'});}catch(error){console.error('RECLAMO_MOVIMIENTO_INICIAL_ERROR',error instanceof Error?error.message:'unknown');}
  try{
    const central=await notificarIngresoReclamoInterno(registro);
    if(!central.enviados) await notificarIngresoInternoLegacy(registro);
  }catch(error){
    console.error('RECLAMO_INTERNAL_SMTP_ERROR',error instanceof Error?error.message:'unknown');
    try{await notificarIngresoInternoLegacy(registro);}catch(legacy){console.error('RECLAMO_INTERNAL_SMTP_LEGACY_ERROR',legacy instanceof Error?legacy.message:'unknown');}
  }
  return registro;
}
