...
export default async function Page({searchParams}:{searchParams:Promise<{caso?:string;pagina?:string}>}){
  ...
  return <AppShell user={u}><div className="space-y-5">
    <section><p className="text-xs font-extrabold tracking-[.18em] text-[#1DB954]">RECLAMOS</p><h1 className="text-2xl font-black text-[#0E2A23]">Bandeja operacional</h1><p className="mt-1 text-sm text-[#6B7570]">Solo aparecen los casos que tu perfil puede ver según la matriz central. Las acciones se habilitan únicamente cuando tienes permiso para solucionar.</p></section>
    <section className="grid min-w-0 gap-4 lg:grid-cols-[minmax(320px,.9fr)_minmax(0,1.4fr)]">
      <div className="space-y-2 rounded-2xl border bg-white p-4"><h2 className="font-black">Casos visibles</h2>{reclamos.map((r:any)=><Link key={r.id} href={`/reclamos-gestion?caso=${r.id}&pagina=${pagina}`} className="block min-w-0 rounded-xl border p-3 hover:bg-[#F6F3EA]"><div className="font-black">R-{String(r.id).padStart(6,'0')} · {r.tipo}</div><div className="truncate text-sm">{r.nombre} · {r.categoria}</div><div className="text-xs text-[#6B7570]">{fechaVisible(r.fecha)} · {r.estado} · {r.area_actual}</div></Link>)}{!reclamos.length&&<p className="rounded-xl bg-[#F6F3EA] p-4 text-sm">No hay reclamos visibles para este perfil.</p>}<div className="flex justify-between pt-2"><Link href={`/reclamos-gestion?pagina=${Math.max(1,pagina-1)}`} className="rounded-lg border px-3 py-2 text-sm font-bold">Anterior</Link><Link href={`/reclamos-gestion?pagina=${pagina+1}`} className="rounded-lg border px-3 py-2 text-sm font-bold">Siguiente</Link></div></div>
