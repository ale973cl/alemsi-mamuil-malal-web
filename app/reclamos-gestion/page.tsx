...
function estadoFinal(estado:unknown){const v=String(estado||'').trim().toLocaleLowerCase('es-CL');return v==='cerrado'||v==='resuelto';}
...
export default async function Page({searchParams}:{searchParams:Promise<{caso?:string;pagina?:string;guardado?:string;detalle?:string;vista?:string}>}){
  const u=await requireUser([...ROLES]);
  const rol=u.rol as RolReclamo;
  const params=await searchParams;
  const pagina=Math.max(1,Number(params.pagina||1));
  const vista=params.vista==='historico'?'historico':'activos';
  const todos=await listarReclamosParaRol(rol,pagina,50);
  const reclamos=todos.filter((r:any)=>vista==='historico'?estadoFinal(r.estado):!estadoFinal(r.estado)).slice(0,25);
  const casoId=Number(params.caso||0);
  const caso=casoId>0?await obtenerDetalleReclamoParaRol(casoId,rol):null;
  const casoFinal=Boolean(caso&&estadoFinal(caso.estado));
  ...
  const puedeGestionar=caso&&!casoFinal?await puedeGestionarReclamo(caso.id,rol):false;
  ...
  const hrefVista=(v:'activos'|'historico')=>`/reclamos-gestion?vista=${v}`;

  return <AppShell user={u}><div className="space-y-5">
    <section><p className="text-xs font-extrabold tracking-[.18em] text-[#1DB954]">RECLAMOS</p><h1 className="text-2xl font-black text-[#0E2A23]">Bandeja operacional</h1><p className="mt-1 text-sm text-[#6B7570]">Los reclamos activos se gestionan aquí. Al resolverse o cerrarse dejan la bandeja operativa y pasan a Histórico, conservando íntegro el expediente y su trazabilidad.</p></section>
    <nav className="flex flex-wrap gap-2 rounded-2xl border bg-white p-2"><Link href={hrefVista('activos')} className={`rounded-xl px-4 py-2 text-sm font-black ${vista==='activos'?'bg-[#0E2A23] text-white':'text-[#0E2A23] hover:bg-[#F6F3EA]'}`}>Reclamos activos</Link><Link href={hrefVista('historico')} className={`rounded-xl px-4 py-2 text-sm font-black ${vista==='historico'?'bg-[#0E2A23] text-white':'text-[#0E2A23] hover:bg-[#F6F3EA]'}`}>Reclamos solucionados / históricos</Link></nav>
    ...
    <section className="grid min-w-0 gap-4 lg:grid-cols-[minmax(320px,.9fr)_minmax(0,1.4fr)]">
      <div className="space-y-2 rounded-2xl border bg-white p-4"><div className="flex items-center justify-between gap-2"><h2 className="font-black">{vista==='historico'?'Casos solucionados / cerrados':'Casos activos visibles'}</h2><span className="rounded-full bg-[#F6F3EA] px-2.5 py-1 text-xs font-black">{reclamos.length}</span></div>{reclamos.map((r:any)=>{
        const propio=String(r.area_actual||'')===rol;
        const derivado=propio&&String(r.estado||'').toLocaleLowerCase('es-CL').includes('derivad');
        return <Link key={r.id} href={`/reclamos-gestion?vista=${vista}&caso=${r.id}&pagina=${pagina}`} className={`block min-w-0 rounded-xl border p-3 transition ${vista==='activos'&&propio?'border-[#0D9B91] bg-[#E8F7F5] shadow-sm':'hover:bg-[#F6F3EA]'}`}>
          <div className="flex flex-wrap items-center justify-between gap-2"><div className="font-black">R-{String(r.id).padStart(6,'0')} · {r.tipo}</div>{vista==='historico'?<span className="rounded-full bg-[#E8F7F5] px-2.5 py-1 text-[11px] font-black text-[#075E58]">{String(r.estado||'RESUELTO').toUpperCase()}</span>:propio&&<span className={`rounded-full px-2.5 py-1 text-[11px] font-black ${derivado?'bg-[#D4AF37]/25 text-[#6D5200]':'bg-[#0D9B91]/15 text-[#075E58]'}`}>{derivado?'DERIVADO A TU PERFIL':'ASIGNADO A TU PERFIL'}</span>}</div>
        ...
        </Link>})}{!reclamos.length&&<p className="rounded-xl bg-[#F6F3EA] p-4 text-sm">{vista==='historico'?'No hay reclamos solucionados o cerrados visibles para este perfil.':'No hay reclamos activos visibles para este perfil.'}</p>}<div className="flex justify-between pt-2"><Link href={`/reclamos-gestion?vista=${vista}&pagina=${Math.max(1,pagina-1)}`} className="rounded-lg border px-3 py-2 text-sm font-bold">Anterior</Link><Link href={`/reclamos-gestion?vista=${vista}&pagina=${pagina+1}`} className="rounded-lg border px-3 py-2 text-sm font-bold">Siguiente</Link></div></div>
      <div className="space-y-4">{caso?<><article className="min-w-0 rounded-2xl border bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-lg font-black">R-{String(caso.id).padStart(6,'0')} · {caso.estado}</h2><span className="rounded-full bg-[#F6F3EA] px-3 py-1 text-xs font-black">Área actual: {caso.area_actual}</span></div><p className="mt-2 text-sm"><b>{caso.tipo} · {caso.categoria}</b></p><p className="mt-1 text-xs text-[#6B7570]">Ingresado: {fechaVisible(caso.fecha)}</p>{casoFinal&&<div className="mt-3 rounded-xl border border-[#0D9B91]/25 bg-[#E8F7F5] p-3 text-sm font-bold text-[#075E58]">Expediente histórico. Se conserva completo para futuras consultas y auditoría.</div>}<p className="mt-2 break-words text-sm">{caso.mensaje}</p>
