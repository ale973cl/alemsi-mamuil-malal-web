...
export default async function Page({searchParams}:{searchParams:Promise<{inicio?:string;fin?:string}>}){
  ...
  return <AppShell user={u}><div className="space-y-5">
    ...
    <section className="grid gap-5 lg:grid-cols-2"><div className="rounded-2xl border bg-white p-5"><h2 className="text-xl font-black">Finanzas</h2><div className="mt-3 space-y-2"><div className="rounded-xl bg-[#F6F3EA] p-3"><b>Pagado período</b><div>${Number(pagadoPeriodo).toLocaleString('es-CL')}</div></div><div className="rounded-xl bg-[#F6F3EA] p-3"><b>Saldo por recaudar</b><div>${Number(saldoTotal).toLocaleString('es-CL')}</div></div></div></div><div className="rounded-2xl border bg-white p-5"><h2 className="text-xl font-black">Producción</h2><div className="mt-3 space-y-2">{d.jornadas.map((r:any)=><div key={r.estado} className="rounded-xl bg-[#F6F3EA] p-3"><b>{r.estado}</b><div>{r.cantidad} jornadas</div></div>)}</div></div></section>

    <section className="rounded-2xl border bg-white p-5"><h2 className="text-xl font-black">Reportes de producción</h2><p className="mt-1 text-sm text-[#6B7570]">Resumen por servicio con acceso al informe consolidado por opción/plato, institución y comensales.</p><div className="mt-3 grid gap-2 md:grid-cols-3">{d.produccion.map((r:any,i:number)=><Link key={i} href={`/produccion/reporte?fecha=${encodeURIComponent(String(r.fecha))}&origen=gerencia`} className="rounded-xl bg-[#F6F3EA] p-3 hover:ring-2 hover:ring-[#0D9B91]/20"><b>{r.fecha} · {r.servicio}</b><div>{r.cantidad} raciones</div><div className="mt-2 text-xs font-black text-[#0D9B91]">Ver informe completo →</div></Link>)}</div></section>
