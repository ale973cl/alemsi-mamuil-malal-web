...
export default async function Page({searchParams}:{searchParams:Promise<{inicio?:string;fin?:string;financiero_desde?:string;financiero_hasta?:string}>}){
  ...
  return <AppShell user={u}><div className="space-y-5">
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{[['Reservado período',reservadoPeriodo],['Pagado período',pagadoPeriodo],['Pendiente período',pendientePeriodo],['Por validar período',porValidarPeriodo],['Deuda anterior acumulada',deudaAnterior],['Saldo total por recaudar',saldoTotal]].map(([label,value])=><div key={String(label)} className="rounded-2xl border bg-white p-4 shadow-sm"><div className="text-sm text-[#6B7570]">{label}</div><div className="mt-1 text-2xl font-black text-[#0E2A23]">${Number(value).toLocaleString('es-CL')}</div></div>)}</section>
    <div className="text-sm text-[#6B7570]">Período financiero: <b>{fechaVisible(periodo.desde)}</b> → <b>{fechaVisible(periodo.hasta)}</b></div>
    <section className="rounded-2xl border bg-white p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-extrabold tracking-[.14em] text-[#1DB954]">MINUTA OFICIAL PUBLICADA</p><h2 className="text-xl font-black">Ciclo operativo viernes → jueves</h2><p className="text-sm text-[#6B7570]">Vista estándar de 7 días · solo lectura. Los días sin carga también se muestran para conservar el período completo.</p></div><form className="flex flex-wrap items-end gap-2"><input type="hidden" name="financiero_desde" value={financieroDesde}/><input type="hidden" name="financiero_hasta" value={financieroHasta}/><label className="text-sm font-bold">Desde <input type="date" name="inicio" defaultValue={inicio} className="ml-1 rounded-lg border p-2"/></label><label className="text-sm font-bold">Hasta <input type="date" name="fin" defaultValue={fin} className="ml-1 rounded-lg border p-2"/></label><button className="rounded-lg border px-3 py-2 font-bold">Consultar</button></form></div><div className="mt-4"><MinutaPublicada rows={minuta as any} compactWeekly inicio={inicio} fin={fin}/></div></section>
