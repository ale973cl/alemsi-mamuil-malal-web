...
export async function POST(req:Request){
  try{
    ...
    let correo:null|Awaited<ReturnType<typeof notificarReservaConfirmada>>=null;
    if(result.email){
      try{
        correo=await notificarReservaConfirmada({
          correo:result.email,
          codigo:result.code,
        ...
        // La reserva ya fue confirmada en PostgreSQL. Un fallo posterior al generar
        // el PDF o enviar el correo no debe responder como si la reserva hubiera fallado.
        console.error('RESERVA_EMAIL_POST_COMMIT_ERROR',error);
      }
    }
