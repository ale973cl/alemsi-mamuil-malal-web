...
import { aplicarMargenRaciones, aplicarMermaCantidad, obtenerParametrosProduccion } from '@/lib/db/produccion-parametros';
...
export async function GET(req:Request){
  ...
  const [produccion,recetas,parametros]=await Promise.all([detalleProduccionFecha(fecha!),listarRecetas(),obtenerParametrosProduccion()]);
  ...
  const grupos=[...new Set(filas.map(r=>`${r.servicio}|||${r.plato}`))].map(k=>{const [servicio,plato]=k.split('|||');const rows=filas.filter(r=>r.servicio===servicio&&r.plato===plato);const receta=activas.find(x=>key(x.plato)===key(plato))||null;const reservas=rows.length;const raciones=aplicarMargenRaciones(reservas,parametros.margen_produccion_pct);return{servicio,plato,reservas,raciones,receta};});
  ...
  const header=()=>{page.drawText('ALEMSI · CASINO MAMUIL',{x:margin,y:y-2,size:13,font:bold,color:NAVY});page.drawText('RECETAS DE PRODUCCIÓN',{x:margin,y:y-20,size:11,font:bold,color:NAVY});page.drawText(`Fecha: ${visibleDate(fecha!)} · Fuente: producción real del día`,{x:margin,y:y-36,size:8,font:regular,color:TEXT});page.drawText(`Margen producción: ${fmt(parametros.margen_produccion_pct)}% · Merma promedio: ${fmt(parametros.merma_promedio_pct)}%`,{x:margin,y:y-48,size:8,font:bold,color:TEAL});page.drawLine({start:{x:margin,y:y-58},end:{x:LETTER[0]-margin,y:y-58},thickness:1.5,color:TEAL});y-=74;};
  ...
  for(const g of grupos){
    need(120);page.drawRectangle({x:margin,y:y-64,width,height:64,color:LIGHT,borderColor:LINE,borderWidth:.7});page.drawText(`${g.servicio.toUpperCase()} · ${g.reservas} RESERVAS · ${g.raciones} RACIONES A PRODUCIR`,{x:margin+10,y:y-18,size:8,font:bold,color:GREEN});
    wrap(g.plato,bold,12,width-20).slice(0,2).forEach((line,i)=>page.drawText(line,{x:margin+10,y:y-38-i*13,size:12,font:bold,color:NAVY}));y-=78;
