...
export async function POST(req:Request){
  try{ const body=await req.json(); const user=await authenticate(String(body.username||""),String(body.password||"")); if(!user)return NextResponse.json({ok:false,error:"Usuario o contraseña no válidos, o cuenta deshabilitada."},{status:401}); await setSession(user); return NextResponse.json({ok:true,user,redirect:HOME_BY_ROLE[user.rol]}); }
