...
export async function reglasAction(fd: FormData) {
  ...
  await setReglas(
    {
      a: Number(fd.get('a') || 48),
      c: Number(fd.get('c') || 24),
      m: Number(fd.get('m') || 7),
      e: fd.get('e') ? 1 : 0,
    },
    u.username
  ...
  await guardarMinuta(
    {
      ...
      tipo_opcion: String(fd.get('tipo_opcion') || ''),
...
export async function enviarAction(fd: FormData) {
  ...
  await enviarCoordinacion(
    String(fd.get('inicio')),
    String(fd.get('fin')),
    u.username
  );
...
export async function publicarAction(fd: FormData) {
  ...
  if (fd.get('confirmar') !== 'PUBLICAR') {
    throw new Error('Debes confirmar la publicación.');
  }

  await publicarMinuta(
    String(fd.get('inicio')),
    String(fd.get('fin')),
    u.username,
    u.rol
...
export async function autorizacionExternaAction(fd: FormData) {
  ...
  await registrarAutorizacionExterna(
    String(fd.get('inicio')),
    String(fd.get('fin')),
    u.username,
    String(fd.get('observacion') || '')
  );
