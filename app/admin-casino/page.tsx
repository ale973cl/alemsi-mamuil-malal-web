...
export default async function Page({searchParams}:{searchParams:Promise<{inicio?:string;fin?:string;tab?:string;caso?:string;pagina?:string}>}){
  ...
  return <AppShell user={u}><div className="space-y-5">
    ...
    {tab==='reglas'&&reg&&<section className="rounded-2xl border border-[#A6B0AA]/25 bg-white p-5"><h2 className="text-xl font-black">Reglas de reserva</h2><p className="mt-1 text-sm text-[#6B7570]">Una configuración alimenta disponibilidad y validación final del servidor.</p><form action={reglasAction} className="mt-4 grid gap-3 md:grid-cols-3"><label className="text-sm font-bold">Modalidad<select name="modalidad" defaultValue={reg.modalidad_cierre||'DIA_COMPLETO'} className="mt-1 w-full rounded-lg border bg-white p-2"><option value="DIA_COMPLETO">Día completo · cierre 00:00 Chile</option><option value="HORAS_EXACTAS">Horas exactas antes del servicio</option><option value="CORTE_DIA_ANTERIOR">Cierre fijo del día anterior</option></select></label><label className="text-sm font-bold">Anticipación Oficina (h)<input type="number" min="0" name="oficina" defaultValue={reg.anticipacion_oficina_horas??reg.anticipacion_reserva_horas} className="mt-1 w-full rounded-lg border p-2"/><span className="mt-1 block font-normal text-[#6B7570]">Se usa en “Horas exactas”.</span></label><label className="text-sm font-bold">Anticipación otros (h)<input type="number" min="0" name="otros" defaultValue={reg.anticipacion_otros_horas??reg.anticipacion_reserva_horas} className="mt-1 w-full rounded-lg border p-2"/><span className="mt-1 block font-normal text-[#6B7570]">Se usa en “Horas exactas”.</span></label><label className="text-sm font-bold">Hora de corte del día anterior<input type="number" min="0" max="23" step="1" name="hora_corte_dia_anterior" defaultValue={reg.hora_corte_dia_anterior??15} className="mt-1 w-full rounded-lg border p-2"/><span className="mt-1 block font-normal text-[#6B7570]">Ejemplo: 15 cierra mañana hoy a las 15:00 (hora de Chile).</span></label><label className="text-sm font-bold">Ventana máxima futura (días)<input type="number" min="1" name="ventana" defaultValue={reg.ventana_maxima_dias??31} className="mt-1 w-full rounded-lg border p-2"/></label><label className="text-sm font-bold">Cancelación (h)<input type="number" min="0" name="c" defaultValue={reg.cancelacion_directa_horas} className="mt-1 w-full rounded-lg border p-2"/></label><label className="text-sm font-bold">Máx. días consecutivos<input type="number" min="1" name="m" defaultValue={reg.max_dias_consecutivos} className="mt-1 w-full rounded-lg border p-2"/></label><label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" name="e" defaultChecked={Number(reg.excepciones_habilitadas)===1}/> Excepciones autorizadas</label><button className="rounded-xl bg-[#1DB954] px-4 py-2 font-extrabold md:col-span-3">Guardar reglas</button></form></section>}
  </div></AppShell>
}
