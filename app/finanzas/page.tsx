...
import FinanzasSubmitButton from '@/components/FinanzasSubmitButton';
      ...
      <div className="mt-5 space-y-3">{visibles.map((r:any)=>{
        ...
        return <details key={r.codigo_reserva} className="rounded-xl border border-[#A6B0AA]/30 p-4">
          ...
          <div className="mt-4 border-t pt-4 text-sm">
            ...
            {accionable&&<div className="mt-3 grid gap-2 lg:grid-cols-2"><form action={pagoAction}><input type="hidden" name="codigo" value={r.codigo_reserva}/><input type="hidden" name="estado" value="Pagado"/><FinanzasSubmitButton pendingLabel="Validando…" className="w-full rounded-lg bg-[#1DB954] px-4 py-2 font-bold">Validar comprobante</FinanzasSubmitButton></form><form action={pagoAction} className="grid gap-2"><input type="hidden" name="codigo" value={r.codigo_reserva}/><input type="hidden" name="estado" value="Rechazado"/><input name="motivo" required placeholder="Motivo obligatorio del rechazo" className="rounded-lg border px-3 py-2"/><FinanzasSubmitButton pendingLabel="Rechazando…" className="rounded-lg border border-[#9B2C2C] px-4 py-2 font-bold">Rechazar</FinanzasSubmitButton></form></div>}
            {!pagado&&!r.comprobante_id&&pendienteSinComprobante(r)&&<div className="mt-4 grid gap-3 lg:grid-cols-[1fr_auto]">
              <details className="rounded-xl border border-[#D4AF37]/50 bg-[#D4AF37]/5 p-3"><summary className="cursor-pointer font-black">Validar sin comprobante · revisión administrativa</summary><form action={validarSinComprobanteAction} className="mt-3 grid gap-2 lg:grid-cols-[220px_1fr_auto]"><input type="hidden" name="codigo" value={r.codigo_reserva}/><select name="medio" required defaultValue="" className="rounded-lg border bg-white px-3 py-2"><option value="" disabled>Medio de verificación</option><option>Cartola bancaria</option><option>Transferencia identificada</option><option>POS / débito</option><option>Validación administrativa</option><option>Otro medio verificable</option></select><input name="motivo" required placeholder="Detalle obligatorio de la verificación" className="rounded-lg border px-3 py-2"/><FinanzasSubmitButton pendingLabel="Procesando pago…" className="rounded-lg bg-[#0E2A23] px-4 py-2 font-black text-white">Marcar pagado y liberar RUT</FinanzasSubmitButton></form></details>
              <form action={solicitarInformacionPagoAction} className="self-start"><input type="hidden" name="codigo" value={r.codigo_reserva}/><FinanzasSubmitButton pendingLabel="Enviando solicitud…" className="w-full rounded-xl border border-[#0D9B91] bg-[#EEF7F6] px-4 py-3 font-black text-[#0E2A23] lg:w-auto">Solicitar información de pago</FinanzasSubmitButton></form>
            </div>}
          </div>
        </details>})}
