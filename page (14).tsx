...
export default async function Page({searchParams}:{searchParams:Promise<{inicio?:string;fin?:string}>}){
  ...
  return <AppShell user={u}><div className="space-y-5">
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      <StatCard label="Almuerzos" value={almuerzos} helper={formatPeriodo(inicio,fin)}/>
    ...
    <SectionCard title="Minuta del período" description={`${dias} día(s) con minuta. Las cantidades al lado de cada plato provienen de las reservas activas.`} actions={<div className="flex flex-wrap items-center gap-2"><PrintButton/><StatusBadge value={flujo?.estado||min[0]?.estado||'Sin flujo'}/><form action={enviarAction}><input type="hidden" name="inicio" value={inicio}/><input type="hidden" name="fin" value={fin}/><button disabled={!min.length} className="rounded-xl border border-[#BFE7D0] bg-[#EFFAF3] px-3 py-2 text-xs font-black text-[#176B42] disabled:opacity-40">Enviar a Coordinación</button></form></div>}>
