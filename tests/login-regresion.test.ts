...
import { auditarSinBloquearLogin, esRolInterno, hashPasswordHistorico, normalizarSesion, SQL_AUDITORIA_ACCESO, usuarioActivo } from '../lib/auth/core.ts';
...
test('auditoría usa solo columnas reales y nunca bloquea el login',async()=>{assert.equal(SQL_AUDITORIA_ACCESO,'INSERT INTO auditoria_acciones (fecha,usuario,accion,detalle) VALUES ($1,$2,$3,$4)');let reportado=false;await assert.doesNotReject(()=>auditarSinBloquearLogin(async()=>{throw new Error('auditoría no disponible');},()=>{reportado=true;}));assert.equal(reportado,true);});
