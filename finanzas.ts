...
export async function listarFinanzas(){
  return query<any>(
    `SELECT s.referencia_reserva,
            ...
            SUM(CASE WHEN COALESCE(s.tipo_registro,'') IN ('CONSUMO_INTERNO','CONSUMO_COORDINADOR') OR LOWER(COALESCE(s.estado_pago,'')) IN ('costo asumido','costo asumido / no cobrable','no aplica') THEN COALESCE(NULLIF(s.precio_aplicado,0),NULLIF(s.precio,0),CASE WHEN COALESCE(i.regla_activa::int,0)=1 AND i.precio_especial IS NOT NULL THEN i.precio_especial ELSE i.precio_dia END,0) ELSE 0 END) costo_asumido_valorizado,
       ...
       LEFT JOIN instituciones i ON i.nombre=s.institucion
       LEFT JOIN LATERAL (
