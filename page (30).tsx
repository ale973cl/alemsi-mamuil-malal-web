...
import { numeroReserva } from '@/lib/ui/format';
...
export default async function ComprobantePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const numero=numeroReserva(reserva.codigo_reserva,reserva.referencia_reserva);
    ...
    <main className="min-h-screen bg-[#F6F3EA] px-4 py-8 sm:py-12">
      <div className="mx-auto max-w-2xl overflow-hidden rounded-3xl border border-[#A6B0AA]/35 bg-[#FFFDF8] shadow-[0_18px_60px_rgba(14,42,35,0.08)]">
        <header className="bg-[#0E2A23] px-6 py-6 text-white">
          ...
          <p className="mt-2 text-sm text-white/70">N.º de reserva {numero}</p>
        </header>
        <div className="p-6">
          {cargado ? (
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-2"><Link href="/mis-reservas" className="rounded-xl bg-[#0E2A23] px-4 py-3 text-center text-sm font-black text-white">← Volver a Mis reservas</Link><Link href="/" className="rounded-xl border border-[#DDE5E2] bg-white px-4 py-3 text-center text-sm font-black text-[#27423B]">Salir del portal</Link></div>
        </div>
      </div>
    </main>
  );
