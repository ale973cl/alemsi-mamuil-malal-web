              ...
              COALESCE(NULLIF(TRIM(s.tipo_opcion),''),
                (SELECT m.tipo_opcion
                  ...
                  WHERE m.fecha=s.fecha AND m.servicio=s.servicio
                    AND m.activo=1 AND COALESCE(m.estado,'PUBLICABLE')='PUBLICABLE'
...
export type RequerimientoTeorico={
  servicio:string;
  tipo_opcion:string;
  plato:string;
  porciones:number;
  insumos:Array<{insumo:string;cantidad:number}>;
};

export async function requerimientoTeoricoFecha(fecha:string):Promise<RequerimientoTeorico[]>{
  const demanda=await demandaFecha(fecha);
  const out:RequerimientoTeorico[]=[];
  for(const row of demanda){
    const recetas=await query<{insumo:string;cantidad:number;merma_pct:number;margen_produccion_pct:number}>(
      `SELECT insumo,cantidad,COALESCE(merma_pct,0) merma_pct,COALESCE(margen_produccion_pct,0) margen_produccion_pct
         FROM recetas
        WHERE LOWER(TRIM(plato))=LOWER(TRIM($1))
          AND UPPER(TRIM(COALESCE(estado,''))) IN ('ACTIVA','ACTIVO','APROBADA','APROBADO')
          AND COALESCE(version,1)=(SELECT MAX(COALESCE(version,1)) FROM recetas WHERE LOWER(TRIM(plato))=LOWER(TRIM($1)) AND UPPER(TRIM(COALESCE(estado,''))) IN ('ACTIVA','ACTIVO','APROBADA','APROBADO'))
        ORDER BY insumo`,[row.plato]
    ).catch(()=>[]);
    out.push({
      servicio:row.servicio,
      tipo_opcion:row.tipo_opcion,
      plato:row.plato,
      porciones:Number(row.reservadas||0),
      insumos:recetas.map(r=>({insumo:r.insumo,cantidad:requerimientoReceta(Number(r.cantidad),Number(row.reservadas||0),Number(r.merma_pct),Number(r.margen_produccion_pct))})),
    });
  }
  return out;
}
